...
const giftSuccessHasPerksTemplate = (gift: Gift) => {
  const perks = JSON.parse(gift.perks as string);
  const shippingInfo = JSON.parse(gift.shippingInfo as string);
      ...
      <div class="body">
        <div>
          <div style="margin: 8px 0;">
            ...
            <span style="font-weight: bold; text-transform: uppercase;">${gift.campaign.title}</span>
          </div>
          <div style="margin: 8px 0;">
            <div>Cảm ơn bạn thời gian qua đã đóng góp cho chiến dịch và là 1 trong những người đóng góp hàng đầu của chiến dịch này</div>
            <div>Chúng tôi rất vui khi được gửi tặng đến bạn một số đặc quyền sau</div>
            <div> 
              <span>Trị giá: </span>
             <span style="font-weight: bold; text-transform: uppercase;">${gift.amount}</span></div>
...
export default giftSuccessHasPerksTemplate;
